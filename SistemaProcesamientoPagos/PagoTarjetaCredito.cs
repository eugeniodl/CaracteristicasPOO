public class PagoTarjetaCredito : MetodoPago
{
    ...
    private string? NumeroTarjeta
    {
        ...
        set
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new ArgumentException("Número de tarjeta requerido");
            if (value.Length < 13 || value.Length > 19)
                throw new ArgumentException("Número de tarjeta inválido");
            _numeroTarjeta = EnmascararNumeroTarjeta(value);
        }
    }

    public PagoTarjetaCredito(string moneda, decimal montoBase, 
    ...
    {
        return $"Tarjeta terminada en {_numeroTarjeta} - Comisión: {CalcularComision():C}";
    }
